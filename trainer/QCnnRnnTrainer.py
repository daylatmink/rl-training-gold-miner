...
def compute_td_loss_dqn(
    ...
    # ----- 4. Target: r_{i+1} + gamma * max_a Q_target(s_{i+1}, a) -----
    with torch.no_grad():
        next_q_max = target_s_ip1.max(dim=-1).values                  # [B, T-1]
        gamma_term = gamma * next_q_max * (~next_is_terminal).float()
...
class QCnnRnnTrainer:
    ...
    def _train_on_batch(self, batch: dict) -> float:
        ...
        with torch.no_grad():
            ...
            next_q_values = self.target_agent.predictor(next_o)  # [B, max_steps, n_actions]
        
        # Compute loss
        loss = compute_td_loss_dqn(q_values=q_values, target_q_values=next_q_values, actions=actions, rewards=rewards, seq_lens=seq_lens, gamma=self.gamma)
